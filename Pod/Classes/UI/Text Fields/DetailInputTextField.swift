...
open class DetailInputTextField: StylizedTextField, TextFieldValidation, AutoCompletingTextField {
    ...
    open func textFieldDidBeginEditing(_ textField: UITextField) {
        ...
        cardInfoTextFieldDelegate?.didStartEditing(textField)
    }
    
    public func textFieldDidEndEditing(_ textField: UITextField) {
        cardInfoTextFieldDelegate?.didEndEditing(textField)
    }
    ...
    open override func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        ...
        var autoCompletedNewText: String? = newText
            if string != "" && newText.count != 2 {
            autoCompletedNewText = autocomplete(newText)
         } else if string != "" && newText.count == 2 {
            autoCompletedNewText = autocomplete(newText)
        }
